...
function calculateUsage(cpuMetricFirst, cpuMetricSecond) {
	let totalTime = (cpuMetricSecond.timestamp - cpuMetricFirst.timestamp) * 1000000;
	const deltaNS = cpuMetricSecond.cpuNanosSinceContainerStart - cpuMetricFirst.cpuNanosSinceContainerStart;
	
	if(totalTime === 0) { // cpuMetricSecond.timestamp and cpuMetricFirst.timestamp somehow were the same
		totalTime = 1;
	}
